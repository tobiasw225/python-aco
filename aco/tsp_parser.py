...
"""
def get_cities(list,dimension,num_cities=-1):
...
def produce_final(file="ulysses16.tsp"):
	...
	# dists = []
	# for pair in tuples:
	# 	heapq.heappush(dists, (pair, distance.euclidean(pair[0], pair[1])))
	# 	# (Stadt-Indices (A,B), und ihre Distance)
    #
	# tau = []
	# tau_zero = 0.1
	# for pair in tuples:
	# 	heapq.heappush(tau, (pair,tau_zero))
	# 	# (Stadt-Indices (A,B) und der Pheromon-Wert auf dem Weg dawischen.
	# print(len(tau))



if __name__ == '__main__':
	produce_final(file="/home/tobias/Dokumente/tsp/rd400.tsp")
